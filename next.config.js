...
module.exports = require('next-plugin-preact')({
  ...
  webpack(config, options) {
    ...
    if (dev && !isServer) {
      ...
      */
      const prefreshPlugin = config.plugins.find(
        (plugin) =>
          plugin.constructor.name === 'ReloadPlugin' &&
          plugin.options?.runsInNextJs
      );

      // entryOptions requires @prefresh/webpack 4.0.2+
      if (
        prefreshPlugin &&
        prefreshPlugin.options.hasOwnProperty('entryOptions')
      ) {
        ...
        config.plugins.unshift({
          apply(compiler) {
            compiler.hooks.thisCompilation.tap('Prefresh', (compilation) => {
              ...
              compilation.globalEntry.options.layer =
